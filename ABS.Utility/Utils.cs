...
namespace CTGroup.Utility
{
    public class Utils
    {
        ...
        //for developement version
        public static string LocalDirectory = "";

        // for live version
        public static string LiveDirectory = "/MISReport";

        public static string ExtDire = IsLive ? LiveDirectory : LocalDirectory;

        public void LogWrite(Exception exception, string filename, int line)
        {
            string filePath = @"C:\Upload\Error.txt";

            using (StreamWriter writer = new StreamWriter(filePath, true))
            {
                writer.WriteLine("-----------------------------------------------------------------------------");
                writer.WriteLine("Date : " + DateTime.Now.ToString());
                writer.WriteLine("File Name: " + filename + " -----------Line No: " + line);
        ...
        public DataTable ExcelToDataTable(HttpPostedFile reqFile, bool UseHeaderRow)
        {
            ...
            try
            {
                ...
                #region Extract Data From Excel

                FileStream = reqFile.InputStream;

                if (reqFile != null && FileStream != null)
                {
                    var actualType = MimeTypes.GetContentType(reqFile.FileName);
                    if (actualType.EndsWith("excel"))
                    {
                        reader = ExcelReaderFactory.CreateReader(FileStream);
                        ds = reader.AsDataSet(new ExcelDataSetConfiguration() { ConfigureDataTable = (_) => new ExcelDataTableConfiguration() { UseHeaderRow = UseHeaderRow } });
                        reader.Close();
        ...
        public DataTable ExcelToDataTable(HttpPostedFile reqFile)
        {
            DataTable dt = new DataTable();
            try
            {
                #region Variable Declaration
                DataSet ds = new DataSet();
                HttpPostedFile InputFile = reqFile;
                IExcelDataReader reader = null;
                Stream FileStream = null;
                #endregion
                ...
                {
                    var actualType = MimeTypes.GetContentType(InputFile.FileName);
                    if (actualType.EndsWith("excel"))
                    {
                        reader = ExcelReaderFactory.CreateReader(FileStream);
                        ds = reader.AsDataSet(new ExcelDataSetConfiguration() { ConfigureDataTable = (_) => new ExcelDataTableConfiguration() { UseHeaderRow = true } });
                        reader.Close();
                        if (ds != null && ds.Tables.Count > 0)
                        {
                            dt = ds.Tables[0];
        ...
        public DataTable ExcelToDataTable(string filePath, bool UseHeaderRow)
        {
            DataTable dt = new DataTable();
            try
            {
                ...
                IExcelDataReader reader = null;
                Stream FileStream = null;
                StreamReader reqFile = null;
                #endregion

                #region Extract Data From Excel
                reqFile = new StreamReader(filePath, System.Text.Encoding.Default); //Data Stream  
                FileStream = reqFile.BaseStream;

                if (reqFile != null && FileStream != null)
                {
                    string fileName = Path.GetFileName(filePath);
                    var actualType = MimeTypes.GetContentType(fileName);
                    if (actualType.EndsWith("excel"))
                    {
                        reader = ExcelReaderFactory.CreateReader(FileStream);
                        ds = reader.AsDataSet(new ExcelDataSetConfiguration() { ConfigureDataTable = (_) => new ExcelDataTableConfiguration() { UseHeaderRow = UseHeaderRow } });
        ...
        public DataTable ExcelStreamToDataTable(HttpPostedFile stream)
        {
            DataTable dt = new DataTable();
            try
            {
                ...
                #region Extract Data From Excel
                reqFile = new StreamReader(stream.InputStream, System.Text.Encoding.Default); //Data Stream  
                FileStream = reqFile.BaseStream;

                if (reqFile != null && FileStream != null)
                {
                    string fileName = Path.GetFileName(stream.FileName);
                    var actualType = MimeTypes.GetContentType(fileName);
                    if (actualType.EndsWith("excel"))
                    {
                        reader = ExcelReaderFactory.CreateReader(FileStream);
        ...
        public string BoolVal(bool? Bool)
        {
            return Bool == true ? "1" : "0";
        }

        public bool BoolVal(string Str)
        {
            return Str == "1" ? true : false;
        }

        public bool IsNotValidActivity(string userId, string nationalId)
        {
            OracleConnection con = new OracleConnection(ConfigurationManager.ConnectionStrings["OracleDbContext"].ConnectionString);
            con.Open();
            string query = "select SPRSN_TEXT from CITYN.T_SPRSN SP WHERE SPRSN_TEXT= '" + userId + "' and sprsn_actv = 'Y'";
            ...
            if (t1.Rows.Count > 0 && (nationalId == null || nationalId == "" || nationalId == "undefined")) // sales team user checking
            {
                return true;
            }
            else
        ...
        public string UploadAssistants(HttpPostedFile hfc, string fileName, string subDirectory)
        {
            string res = string.Empty, url = "https://app.citygroupbd.com:222/api/DMSFileUpload/formcollection";
            MultipartFormDataContent form = new MultipartFormDataContent();
            HttpContent fileContent = new StreamContent(hfc.InputStream);
            form.Add(fileContent, "Reqfiles", hfc.FileName.ToString());
            form.Add(new StringContent(subDirectory), "subDirectory");
        ...
        public string UploadAssistant(HttpPostedFile hfc, string fileName, string subDirectory)
        {
            string res = string.Empty, url = "https://app.citygroupbd.com:222/api/DMSFileUpload/formcollection";
            MultipartFormDataContent form = new MultipartFormDataContent();
            HttpContent fileContent = new StreamContent(hfc.InputStream);
        ...
        public string UploadAssistant_Test(HttpPostedFile hfc, string fileName, string subDirectory)
        {
            string res = string.Empty, url = "http://localhost:49942/api/docupload/uploadforms";
            MultipartFormDataContent form = new MultipartFormDataContent();
            HttpContent fileContent = new StreamContent(hfc.InputStream);
            form.Add(fileContent, "Reqfiles", hfc.FileName.ToString());
            form.Add(new StringContent(subDirectory), "subDirectory");
        ...
        public DataTable FileStreamExcelToDataTable(byte[] fileByte, string fileName)
        {
            DataTable dt = new DataTable();
            try
            {
                if (fileByte != null)
                {
                    ...
                    //StreamReader reqFile = null;
                    #endregion
                    FileStream = new MemoryStream(fileByte);
