    ...
    function ($scope, crdMyProfileSerive) {
        ...
        $scope.ChangePaswordSave = function () {
            ...
            var AuthenticateUser = {
                EmpID: LUserID,
                UserPassw: $scope.txtbxNewPassword,
                UserPrevPass: $scope.txtCurrentPassword
            };

            var urlLogin = 'https://cssap.citygroupbd.com/acmsapi/api/usersetup/changepassword';
                            
            var LoginUser = crdMyProfileSerive.postExternal(urlLogin, AuthenticateUser);
            LoginUser.then(function (response) {
                debugger
                if (response.data === '1') {
                    Command: toastr["info"]("Password changed successfully!!!!");
