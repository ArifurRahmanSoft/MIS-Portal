...
app.controller('sideNavCtrl', ['$scope', 'hostDire', 'sideNavService', '$localStorage', '$rootScope',
    function ($scope, hostDire, sideNavService, $localStorage, $rootScope) {
        ...
        function loadRecords_Top(dataModuleID) {
            $.ajax({
                method: "get",
                url: hostDire + '/SystemCommon/api/SystemCommonLayout/GetTopMenu/' + $scope.sessionUserCompanyID + '/' + $scope.sessionLoggedUserID + '/' + $scope.sessionUserCompanyID,
                async: false,
                dataType: "json",
