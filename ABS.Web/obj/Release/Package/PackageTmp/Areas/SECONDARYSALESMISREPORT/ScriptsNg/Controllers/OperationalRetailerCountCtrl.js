app.controller('operationalRetailerCountCtrl', ['$scope', '$http', '$window', 'crudService', 'conversion', '$filter', '$localStorage', 'uiGridConstants',
    function ($scope, $http, $window, crudService, conversion, $filter, $localStorage, uiGridConstants) {
        ...
        var secApiUrl = '/SECONDARYSALESMISREPORT/api/SecondaryReportCommon/';
        ...
        $scope.StartDate = conversion.DateCustom(31);
        $scope.EndDate = conversion.DateCustom(1);
        ...
        $scope.GenDate = '';
        $scope.GetLastGenDate = function () {
            debugger;
            var param = [{ fromDate: $scope.StartDate, toDate: $scope.EndDate }];
            var apiRoute = secApiUrl + 'GetLastGenDate/';
            var listModel = crudService.postMultipleModel(apiRoute, param, $scope.HeaderToken.get);
            listModel.then(function (response) {
                debugger;
                $scope.GenDate = response.data.res;
            },
                function (error) {
                    console.log("Error: " + error);
                });
        }
        $scope.GetLastGenDate();

        $scope.isGen = false;
        $scope.GenerateOrdDlvCountCustom = function () {
            debugger;
            $scope.isGen = true;
            var param = [{ fromDate: $scope.StartDate, toDate: $scope.EndDate }];
            var apiRoute = secApiUrl + 'GenOrdDlvCountCustom/';
            var listModel = crudService.postMultipleModel(apiRoute, param, $scope.HeaderToken.get);
            listModel.then(function (response) {
                debugger;
                $scope.GenDate = response.data.res;
        ...
        $scope.ShowProductTransactionReport = function (reportType) {
            ...
            var queryStringPTR = commonQueryString + ',' + $scope.ngModelBrand + ',' + $scope.ngModelProduct + ',' + reportType + ',' + $scope.reportcategory;
