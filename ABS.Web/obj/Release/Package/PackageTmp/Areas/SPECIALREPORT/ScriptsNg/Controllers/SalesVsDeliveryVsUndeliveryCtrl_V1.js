app.controller('SalesVsDeliveryVsUndeliveryCtrl', ['$scope', '$http', '$window', 'crudService', 'conversion', '$filter', '$localStorage', 'uiGridConstants',
    function ($scope, $http, $window, crudService, conversion, $filter, $localStorage, uiGridConstants) {
       ...
       // $scope.loadUserWiseReportPermission = function (isPaging) {
        ...
        $scope.ShowReport = function () {
            var reportType = $scope.ReportType == undefined ? '' : $scope.ReportType;
            ...
            var queryStringPTR = commonQueryString + ',' + brandId + ',' + productId + ',' + reportType;

            var reportPath = repBaseUrl + 'SalesVsDeliveryVsUndelivery/SalesVsDeliveryVsUndelivery.aspx?queryString=' + queryStringPTR;
            conversion.reportOpen(reportPath);
            //$window.open(reportPath);
        };          

        $scope.ShowMonthwiseSalesDelivery = function () {
            debugger
            // national filter
            if ($scope.ngModelNational != 'SNTNLxxxxxxxxxN00001') {
                Command: toastr["warning"]("Please select consumer national !!!");
                return;
            }
            // division filter
            if ($scope.ngModelDivision != 'SDVSNxxxxxxxxxD00012' && $scope.ngModelDivision != 'SDVSNxxxxxxxxxD00001') {
                Command: toastr["warning"]("Please select appropriate divisions !!!");
                return;
            }
            // date validation
            var fixedFirstDate = conversion.getStringToDate('01/07/2020');
            fixedFirstDate = new Date(fixedFirstDate);
            var selectedFirstDate = conversion.getStringToDate($scope.StartDate);
            selectedFirstDate = new Date(selectedFirstDate);
            if (selectedFirstDate < fixedFirstDate) {
                Command: toastr["warning"]("Please select your authorized duration !!!");
                return;
            }
            /// to date validation
            var fixedSecondDate = conversion.getStringToDate('31/08/2022');
            fixedSecondDate = new Date(fixedSecondDate);
            var selectedSecondDate = conversion.getStringToDate($scope.EndDate);
            selectedSecondDate = new Date(selectedSecondDate);
            if (selectedSecondDate > fixedSecondDate) {
                Command: toastr["warning"]("Please select your authorized duration !!!");
                return;
            }
            ...
            var reportPath = repBaseUrl + 'MonthwiseSalesDelivery/MonthwiseSalesDelivery.aspx?queryString=' + queryString;
            conversion.reportOpen(reportPath);
            //$window.open(reportPath);
        };       
