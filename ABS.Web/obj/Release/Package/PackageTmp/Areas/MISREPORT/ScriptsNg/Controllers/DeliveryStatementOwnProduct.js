app.controller('deliveryStatementOwnProduct', ['$scope', '$http', '$window', 'crudService', 'conversion', '$filter', '$localStorage', 'uiGridConstants',
    function ($scope, $http, $window, crudService, conversion, $filter, $localStorage, uiGridConstants) {
        ...
        $scope.isDisableProdCom = true;
        $scope.productCompanyId = '';
        $scope.listProductCompany = [];
        $scope.GetUserWiseProductCompanyList = function () {
            $scope.listProductCompany = [];
            $scope.productCompanyId = '';
            $("#productCompanyId").select2("data", { id: '', text: '--Select Product Company--' });
            $scope.cmnParam();
            var param = { loggeduser: objcmnParam.loggeduser };
            var apiRoute = sysCommonUrl + 'GetUserWiseProductCompany/';
            var listModel = crudService.getItemWithcmnParam(apiRoute, param);
            listModel.then(function (response) {
                $scope.listProductCompany = response.data.prodComList;
                $scope.isDisableProdCom = $scope.listProductCompany.length > 1 ? false : true;
                if ($scope.listProductCompany.length == 1) {
                    $scope.productCompanyId = $scope.listProductCompany[0].SCOMP_OID;
                    $("#productCompanyId").select2("data", { id: $scope.listProductCompany[0].SCOMP_OID, text: $scope.listProductCompany[0].SCOMP_NAME });
                }
            },
                function (error) {
                    console.log("Error: " + error);
        ...
        $scope.GetUserWiseProductCompanyList();
        ...
        $scope.ShowDeliveryStatementOwnProduct = function () {
            debugger
            if ($scope.ReportType == '' || $scope.ReportType == undefined) {
                Command: toastr["warning"]("Please check report type !!!");
                return;
            }
            var queryStringUDS = commonQueryString + ',' + $scope.ngModelBrand + ',' + $scope.ngModelProduct + ',' + $scope.ngModelCompany + ',' + $scope.ngModelLocation + ',' + $scope.productCompanyId + ',' + $scope.ReportType;

            var reportPath = repBaseUrl + 'DeliveryStatementOwnProduct/DeliveryStatementOwnProduct.aspx?queryString=' + queryStringUDS;
            conversion.reportOpen(reportPath);
            //$window.open(reportPath);
        };
        ...
        $scope.clearDeliveryStatementOwnProduct = function () {
            //$scope.formUndeliveredStatement.$setPristine();
            //$scope.formUndeliveredStatement.$setUntouched();
