app.controller('brandWiseSalesStatementCtrl', ['$scope', '$http', '$window', 'crudService', 'conversion', '$filter', '$localStorage', 'uiGridConstants',
    function ($scope, $http, $window, crudService, conversion, $filter, $localStorage, uiGridConstants) {
            ...
            listModel.then(function (response) {
                ...
                if (userrole.length > 0) {
                    ...
                    listPermission.then(function (response) {
                        ...
                        if ($scope.listPermission.length > 0) {
        ...
        // Get all distributor by zone --- means after selecting a zone  
        ...
        $scope.loadProductGroup = function () {
            if (!$scope.isBrandGroup) {
                $scope.listProdGroup = [];
                $scope.ngModelProdGroup = '';
                $("#ddlProductGroup").select2("data", { id: '', text: '--Select Prodouct Group--' });

                objcmnParam.nationalId = $scope.ngModelNational == "" || $scope.ngModelNational == undefined ? null : $scope.ngModelNational;
        ...
        $scope.loadBrandByNational = function () {
            if (!$scope.isBrandGroup) {
                ...
                var mode = '1';
                var trackNo = $scope.ngModelNational == "" || $scope.ngModelNational == undefined ? null : $scope.ngModelNational;
        ...
        // Common Query String for Report - Use for all reports where used area hierarchy
        ...
        $scope.ShowBrandWiseSalesStatement = function (data) {
            ...
            var groupId = $scope.ngModelProdGroup == undefined ? '' : $scope.ngModelProdGroup;
            ...
            var queryStringBWSS = commonQueryString + ',' + reportOption + ',' + brandId + ',' + productId + ',' + groupId + ',' + isBrandGroup + ',' + brandGroupId + ',' + companyId;
        ...
        $scope.clear = function () {
            ...
            $scope.ListSKUBrand = [];
            $scope.ngModelProduct = '';
            $("#ddlProduct").select2("data", { id: '', text: '--Select Brand--' });
