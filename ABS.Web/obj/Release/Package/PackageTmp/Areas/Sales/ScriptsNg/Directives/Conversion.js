...
app.service('conversion', function ($filter, $window, hostDire) {
    ...
    this.YearMonthDateFormat = function (InputString) {
        //debugger
        var newStringToDate = InputString.replace(/[/-]/g, '-');
        var StringToDate = newStringToDate;
        var SplitedDate = StringToDate.split("-");
        ...
        var Month = SplitedDate[1];
        var Year = SplitedDate[2];
        var FullFormateDate = Year + "-" + Month + "-" + Day;
        var Output = $filter('date')(new Date(), FullFormateDate);
        return Output;
    }
    ...
    this.UniformUnChecked = function (elementid) {
        $('#uniform-' + elementid + '').find('span:first').removeClass('checked');
    }
    ...
    this.LastDayOfMonth = function () {
        ...
        var lastDay = ('0' + lDay.getDate()).slice(-2) + '/' + ('0' + (lDay.getMonth() + 1)).slice(-2) + '/' + lDay.getFullYear();
        return lastDay;
    }
    ...
    this.FirstDayOfMonth = function () {
        ...
        var firstDay = ('0' + fDay.getDate()).slice(-2) + '/' + ('0' + (fDay.getMonth() + 1)).slice(-2) + '/' + fDay.getFullYear();
        return firstDay;
    }
    ...
    this.reportOpen = function (repUrl) {
        repUrl = hostDire + repUrl;
        $window.open(repUrl);
    }
    ...
    this.ExcelToArray = function (ExcelFile, CEntitys) {
        debugger;
        if (ExcelFile != null) {
            var regex = /^([a-zA-Z0-9\s_\\.\-:])+(.xls|.xlsx)$/;
            if (regex.test(ExcelFile.name.toLowerCase())) {
                if (typeof (FileReader) != "undefined") {
                    var reader = new FileReader();
                    //For Browsers other than IE.
                    if (reader.readAsBinaryString) {
                        reader.onload = function (e) {
                            ProcessExcel(e.target.result, CEntitys);
                        };
    ...
    ProcessExcel = function (data, CEntitys) {
        ...
        CEntitys.ExcelRows = XLSX.utils.sheet_to_row_object_array(workbook.Sheets[firstSheet]);
    };

    this.downloadDocument = function (url, fileName) {
        debugger;
        fetch(url)
