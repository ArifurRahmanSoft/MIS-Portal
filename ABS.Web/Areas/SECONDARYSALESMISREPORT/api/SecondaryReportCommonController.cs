...
namespace CTGroup.Web.Areas.SECONDARYSALESMISREPORT.api
{
    [RoutePrefix("SECONDARYSALESMISREPORT/api/SecondaryReportCommon")]
    public class SecondaryReportCommonController : ApiController
    {
        private iDistributorTargetMgt objPIService = null;

        public SecondaryReportCommonController()
        {
        ...
        [HttpPost, BasicAuthorization]
        public IHttpActionResult GetLastGenDate(object[] data)
        {
            string res = string.Empty;
            try
            {
                res = objPIService.GetLastGenDate();
            }
            catch (Exception e)
            {
                e.ToString();
        ...
        [HttpPost, BasicAuthorization]
        public IHttpActionResult GenOrdDlvCountCustom(object[] data)
        {
            string res = string.Empty;
            vmCmnParameters objcmnParam = JsonConvert.DeserializeObject<vmCmnParameters>(data[0].ToString());
            try
            {
                res = objPIService.GenOrdDlvCountCustom(objcmnParam);
            }
            catch (Exception e)
            {
                e.ToString();
