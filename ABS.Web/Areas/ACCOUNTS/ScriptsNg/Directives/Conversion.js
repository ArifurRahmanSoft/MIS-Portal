...
app.service('conversion', function ($filter, $window, hostDire) {
    ...
    this.loadCurrentMonth = function (Year) {
        debugger
        var currentMonths = [];
        months.forEach(function (month, index) {
            let startDate = `01/${month.number}`;
              currentMonths.push({
                id: startDate,
                text: month.name
              });
    ...
    this.loadEndMonth = function (toYear) {
        debugger
        ...
        months.forEach(function (month, index) {
            ...
            let lastDay = new Date(nToYear, index + 1, 0).getDate();
            endDate = `${lastDay}/${month.number}`;
            //endDate = `${month.number}/${lastDay}`;
                endMonths.push({
                id: endDate,
                text: month.name
            });
        });
        console.log("endMonths",endMonths);
        return endMonths;
    };
