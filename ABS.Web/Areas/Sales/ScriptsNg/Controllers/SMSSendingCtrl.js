...
app.controller('SMSSendingCtrl', ['$scope', 'crudService', 'uiGridConstants', '$q', '$http', 'conversion', '$filter', '$localStorage',
    function ($scope, crudService, uiGridConstants, $q, $http, conversion, $filter, $localStorage) {

        var baseUrl = "/Sales/api/SMSSending/";
        ...
        $scope.btnSaveText = "Send";
        $scope.btnShowText = "Show List";

        $scope.PageTitle = 'SMS Sending (Single)';
        $scope.ListTitle = 'SMS Sending (Single)';

        $scope.PageTitle = 'SMS Sending (Single)';
        $scope.ListTitle = 'SMS Sending (Single)';
        $scope.ListTitleMrrMasters = 'SMS Sending (Single)';
        ...
        function loadMotherVesselMasterRecords(isPaging) {
            ...
            $scope.gridOptionATRS = {
                columnDefs: [
                    ...
                    { name: "MESSAGETEXT", displayName: "MESSAGE TEXT", title: "MESSAGE TEXT", width: '45%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "MOBILENUMBER", displayName: "MOBILE NUMBER", title: "MOBILE NUMBER", width: '23%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "ENTRYUSER", displayName: "SENT BY", title: "SENT BY", width: '15%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "ENTRYDATE", displayName: "SENT TIME", title: "SENT TIME", width: '15%', cellFilter: 'date:"dd-MM-yyyy"', headerCellClass: $scope.highlightFilteredHeader }
                    //{
                    //    name: 'Action',
                    //    displayName: "Action",
                    //    pinnedRight: true,
                ...
                exporterCsvFilename: 'BulkClosingBalance.csv',
            ...
            var apiRoute = baseUrl + 'GetSentMessageDetail/';
        ...
        $scope.GetDetail = function (datamodel) {
            ...
            $scope.BULKDAILYCLOSINGBALANCE = datamodel.CLOSINGBALANCE;
        };
        ...
        $scope.Save = function () {
            ...
            if ($scope.MESSAGETEXT == '' || $scope.MESSAGETEXT == undefined || $scope.MOBILENUMBER == '' || $scope.MOBILENUMBER == undefined) {
                Command: toastr["warning"]("Please input mobile number and your message !");
                return;
            }

            if ($scope.MOBILENUMBER.length != 11) {
                Command: toastr["warning"]("Mobile Number must be 7 digit !");
                return;
            }
            ...
            var apiRoute = baseUrl + 'SendSingleSMS/';
            var UserCreate = crudService.postList(apiRoute, Collector);
            UserCreate.then(function (response) {          
                if (response.data === "1") {
                    Command: toastr["info"]("Message Sent Successfully !!!");
                    modal_fadeOut();
                    $scope.clear();
                }
                else {
                    Command: toastr["error"]("Message Sending Failed !!!");
                }
            },
                function (error) {
                    console.log("Error: " + error);
