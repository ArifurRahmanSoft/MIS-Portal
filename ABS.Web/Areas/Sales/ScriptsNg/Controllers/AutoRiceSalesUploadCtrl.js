app.controller('AutoRiceSalesUploadCtrl', ['$scope', 'hostDire', 'crudService', 'conversion', '$http', '$filter', '$localStorage', 'uiGridConstants',
    function ($scope, hostDire, crudService, conversion, $http, $filter, $localStorage, uiGridConstants) {

        var baseUrl = "/Sales/api/AutoRiceSalesUpload/";
        ...
        $scope.PageTitle = 'Auto Rice Sales Upload';
        $scope.DocumentMasterList = 'Auto Rice Sales Upload Information';
        ...
        //$scope.downloadDocument = function (url, fileName) {
        //    debugger;
        //    fetch(url)
        ...
        $scope.getExcelRows = function (ExcelRows, isView) {
            ...
            if (ExcelRows.length > 0) {
                var undeFinedCol = ExcelRows.filter(x => x.SALES_ORG == undefined || x.CUSTOMER_CODE == undefined || x.SALES_DATE == undefined || x.SALES_ORDER == undefined || x.MATERIAL_NAME == undefined || x.MATERIAL_DESCRIPTION == undefined || x.QNT_IN_BAG_CTN == undefined || x.QNT_IN_KG_PCS == undefined || x.QNT_IN_TON == undefined || x.NET_WEIGHT == undefined || x.SALES_VALUE == undefined || x.SALES_TYPE == undefined);
                if (undeFinedCol.length == 0) {
                    angular.forEach(ExcelRows, function (item) {
                        modifiedExcelRows.push({
                            SALESORG: item.SALES_ORG, CUSTOMERCODE: item.CUSTOMER_CODE, SALESDATE: item.SALES_DATE, SALESORDERNO: item.SALES_ORDER,
                            MATERIALNAME: item.MATERIAL_NAME, ITEMNAME: item.MATERIAL_DESCRIPTION, QUANTITYBAG: item.QNT_IN_BAG_CTN, QUANTITYKGPC: item.QNT_IN_KG_PCS,
                            QUANTITYTON: item.QNT_IN_TON, NETWEIGHT: item.NET_WEIGHT, VALUE: item.SALES_VALUE, SALESTYPE: item.SALES_TYPE
                        });
                    });
        ...
        $scope.loadExcelUploadedDataList = function (dataList) {
            ...
            $scope.gridOptionsPreviewData = {
                columnDefs: [
                    { name: "SALESORG", displayName: "Sales Org", width: "7%", visible: true, headerCellClass: $scope.highlightFilteredHeader },
                    { name: "CUSTOMERCODE", displayName: "Customer Code", width: "8%", visible: true, headerCellClass: $scope.highlightFilteredHeader },
                    { name: "SALESDATE", displayName: "Date", width: "5%", title: "Date", cellFilter: 'date:"dd-MM-yyyy"', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "SALESORDERNO", displayName: "Sales Order", width: "10%", title: "Sales Order", headerCellClass: $scope.highlightFilteredHeader },
                    { name: "MATERIALNAME", displayName: "Material Name", width: "8%", title: "Material Name", headerCellClass: $scope.highlightFilteredHeader },
                    { name: "ITEMNAME", displayName: "Item Description", width: "20%", title: "ITEMID", headerCellClass: $scope.highlightFilteredHeader },
                    { name: "QUANTITYBAG", displayName: "Quantity (BAG)", width: "8%", title: "Quantity (KG/PCS)", visible: true, headerCellClass: $scope.highlightFilteredHeader },
                    { name: "QUANTITYKGPC", displayName: "Quantity (KG/PCS)", width: "10%", title: "Quantity (Bag)", visible: true, headerCellClass: $scope.highlightFilteredHeader },
                    { name: "QUANTITYTON", displayName: "Quantity (TON)", width: "8%", title: "Quantity (TON)", visible: true, headerCellClass: $scope.highlightFilteredHeader },
                    { name: "NETWEIGHT", displayName: "Net Weight", width: "8%", title: "Net Weight", visible: true, headerCellClass: $scope.highlightFilteredHeader },
                    //{ name: "RATE", displayName: "Rate", width: "10%", title: "Rate", visible: true, headerCellClass: $scope.highlightFilteredHeader },                    
                    { name: "VALUE", displayName: "Value", width: "8%", title: "Value", visible: true, headerCellClass: $scope.highlightFilteredHeader },
                    { name: "SALESTYPE", displayName: "Sales Type", width: "8%", title: "Value", visible: true, headerCellClass: $scope.highlightFilteredHeader }
                ],
                enableFiltering: true,
                enableGridMenu: true,
                enableSelectAll: true,
