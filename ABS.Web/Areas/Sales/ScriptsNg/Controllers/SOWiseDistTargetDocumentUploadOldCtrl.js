app.controller('SOWiseDistTargetDocumentUploadCtrl', ['$scope', 'hostDire', 'crudService', 'conversion', '$http', '$filter', '$localStorage', 'uiGridConstants',
    function ($scope, hostDire, crudService, conversion, $http, $filter, $localStorage, uiGridConstants) {

        var baseUrl = '/Sales/api/SOWiseDocumentUpload/';
        ...
        $scope.Save = function () {
            ...
            if ($scope.ListTargetUploadedData.length > 0) {
                var HeaderTokenPutPost = $scope.HeaderToken.post;
                ModelsArray = [$scope.ListTargetUploadedData, objcmnParam];
                var apiRoute = baseUrl + 'UploadDocumentsSaveSOWiseDistTarget/';
