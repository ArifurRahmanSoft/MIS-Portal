app.controller('SOWiseDistTargetUploadCtrl', ['$scope', 'hostDire', 'crudService', 'conversion', '$http', '$filter', '$localStorage', 'uiGridConstants',
    function ($scope, hostDire, crudService, conversion, $http, $filter, $localStorage, uiGridConstants) {

        var baseUrl = '/Sales/api/SOWiseDistTargetUpload/';
        ...
        $scope.PageTitle = 'SO-Wise Distributor Target Upload';
        $scope.DocumentMasterList = 'SO-Wise Distributor Target Uploaded List';
        ...
        $scope.docfile = [];
        $scope.isDocFile = '';
        $scope.StartDate = conversion.FirstDayOfMonth();
        $scope.EndDate = conversion.LastDayOfMonth();
        //***************************************************End Vairiable Initialize***************************************************
        //$scope.CmnMethod = function (FuncEntity, CmnNum) { $scope.CmnEntity = {}; $scope.CmnEntity = conversion.ExecuteCmnFunc(FuncEntity, CmnNum); if (CmnNum != 0 && CmnNum != 2) { $scope[$scope.CmnEntity.MethodName]($scope.CmnEntity.rowEntity); } if (CmnNum == 2) { for (var i = 0; i < $scope.CmnEntity.MethodName.length; i++) { $scope[$scope.CmnEntity.MethodName[i]]($scope.CmnEntity.rowEntity); } } if (CmnNum == 3) { $scope.cmnbtnShowHideEnDisable(num = (toast = $('.toast-warning').attr("style")) == undefined ? $scope.CmnEntity.num : 7); } }
        //***************************************************Start Common Task for all**************************************************
        frmName = 'frmTargetUpload'; DelFunc = 'DeleteBuildingMasterRecord'; DelMsg = 'DocName'; EditFunc = 'getDocumentListByID';
        ...
        $scope.loadDistTargetUploadList = function (isPaging) {
            ...
            $scope.gridOptionsDTU = {
                ...
                columnDefs: [
                    ...
                    { name: "DOCUMENTNAME", displayName: "FILE NAME", width: "35%", title: "FILE NAME", headerCellClass: $scope.highlightFilteredHeader },
                    { name: "NTNAL_NAME", displayName: "NATIONAL", width: "35%", title: "NATIONAL", headerCellClass: $scope.highlightFilteredHeader },
                    { name: "START_DATE", displayName: "START DATE ", width: "25%", title: "START DATE ", cellFilter: 'date:"dd-MM-yyyy"', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "END_DATE", displayName: "END DATE ", width: "25%", title: "END DATE ", cellFilter: 'date:"dd-MM-yyyy"', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "VIRTUALPATH", displayName: "VIRTUAL PATH", width: "5%", title: "VIRTUAL PATH", visible: false, headerCellClass: $scope.highlightFilteredHeader },
                    {
                        ...
                        cellTemplate: '<span class="label label-info label-mini">' +
                            ...
                            '<i class="icon-check" aria-hidden="true"></i> Cancel' +
                            '</a>' +
                            '</span>'
                    }
                ],
        ...
        $('#file').change(function () {
            ...
            $scope.$apply(function () {
                $scope.docfile = document.getElementById('file').files;                
            });
        ...
        $scope.ViewFileData = function () {
            if (conversion.YearMonthDateFormat($scope.StartDate) > conversion.YearMonthDateFormat($scope.EndDate)) {
                Command: toastr["warning"]("Target Start Date can not be greater than End Date !!!");
                return;
            }
            ...
            if ($scope.docfile.length > 0) {
                $scope.uploadDocumentFileView();
            }
            else {
                Command: toastr["warning"]("No file chosen to view data !!!");
                $scope.IsLoadingIcon = false;                
            }
        }

        $scope.ListColumn = [];
        $scope.ListTargetUploadedData = [];

        $scope.ListTargetUploadedDataDisplay = [];

        $scope.uploadDocumentFileView = function () {
            var formData = new FormData();
            angular.forEach($scope.docfile, function (file, index) {
                debugger;
                var fls = $scope.docfile[index];
                formData.append("uploadedFile", $scope.docfile[index]);
            });

            formData.append("nationalid", $scope.nationalId);
            ...
            $http.post(upURL + "UploadDocumentsView/", formData,
                ...
                .success(function (response) {
                    ...
                    if (response.result == "1") {
                        ...
                        //Only Display
                        $scope.ListColumn = response.colList;
                        $scope.ListTargetUploadedDataDisplay = response.excelDt;
                        //Only Display

                        //Actual Data to Save
                        $scope.ListTargetUploadedData = response.sProdList;
                        ...
                        debugger
                        /*alert(response.dataMismatch);*/
                        alert(response.dataMismatch.m_StringValue);
                        $scope.IsLoadingIcon = false;
        ...
        $scope.nationalId = '';
        $scope.listNational = [];
        $scope.loadAllNational = function () {
            debugger;
            var cmnParam = { strId: '0' };
            $scope.listNational = [];
            $scope.nationalId = '';
            $("#nationalId").select2("data", { id: '', text: '--Select National--' });
            var apiRoute = sysCommonUrl + 'GetAllNational/';
            var ntnalRecords = crudService.postMultipleModels(apiRoute, cmnParam, $scope.HeaderToken.get);
            ntnalRecords.then(function (response) {
                $scope.listNational = response.data.nationalList;
            },
                function (error) {
                    console.log("Error: " + error);
                });
        }
        $scope.loadAllNational();
        //Get All National

        $scope.Save = function () {
            $scope.cmnParam();
            if ($scope.ListTargetUploadedData.length > 0) {
                var HeaderTokenPutPost = $scope.HeaderToken.post;
                ModelsArray = [];
                var apiRoute = baseUrl + 'FinalSaveToUploadSoWiseTarget/';
            ...
            else if ($scope.ListTargetUploadedData.length <= 0) {
        ...
        $scope.clear = function () {
            $scope.frmTargetUpload.$setPristine();
            $scope.frmTargetUpload.$setUntouched();
            $scope.IsLoadingIcon = false;
        ...
        $scope.DeleteDistributorTargetRecord = function (dataModel) {
            ...
            var IsConf = confirm('You are about to cancel SO-Wise Distributor Target data from '
                + conversion.getDateToString(dataModel.START_DATE) + ' to '
                + conversion.getDateToString(dataModel.END_DATE) + '. This operation can not be undone. Are you sure?');
            ...
            if (IsConf) {
                ...
                documentDelete.then(function (response) {
                    ...
                    if (response.data.result == '1') {
                        ...
                        Command: toastr["success"]("Your selected target records has been cancelled successfully!");
                        $scope.IsLoadingIcon = false;
                    }
                    else {
                        $scope.IsLoadingIcon = false;
                        Command: toastr["warning"]("Selected record has not been cancelled, Please Check and Try Again!");
                    }
                }, function (error) {
                    $scope.IsLoadingIcon = false;
                    Command: toastr["warning"]("Selected record has not been cancelled, Please Check and Try Again!");
