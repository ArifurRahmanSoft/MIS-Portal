...
app.controller('ProductSetupCtrl', ['$scope', 'crudService','conversion', '$filter', '$localStorage', 'uiGridConstants',
  ...
  //-----------------------------------------------------Start--------------------------------------
       //GET PRODUCT LIST
        ...
        $scope.GetDetailsByProductCode = function (product) {
            ...
            if ($scope.ProductDetail.length > 0) {
                ...
                existingProduct = $scope.ProductDetail.find(p => p.sprod_name === product.sprod_name);
        ...
        $scope.SaveProductDetail = function () {
            debugger
            var IsConf = confirm('Are you sure You Want to Save ?' );
            if (IsConf) {
       ...
       /*     $("#ddlProductBrand").select2("data", { id: '', text: '--Select Brand--' });*/
        ...
        $scope.GetLitemById = function (prod) {
            ...
            return crudService.getListByParam(apiRoute, proCode)
                .then(function (response) {
                    console.log("litem litemlitemlitem response", response);
                    if (response.data.resdata) {
                        let lItemLst = JSON.parse(response.data.resdata);

                        console.log("lItemLst", lItemLst);

                        // return first item if exists
                        if (lItemLst && lItemLst.length > 0) {
                            return lItemLst[0];
                        }
}
        ...
        $scope.GetDetailsByLitemCode = function (prod) {
            ...
            // call api
            $scope.GetLitemById(prod).then(function (litem) {

                console.log("litem litemlitemlitem", litem );
        ...
        $scope.SavelItemDetails = function () {
            ...
            lItemSaveUpdate.then(function (response) {
                ...
                if (response.data === "exist") {
                    ShowCustomToastrMessageResult(-1, $scope.UserCommonEntity);
                         $scope.lItem = [];
                         return;
                }
