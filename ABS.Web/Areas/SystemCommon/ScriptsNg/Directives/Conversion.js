...
app.service('conversion', function ($filter, $window, hostDire) {
    ...
    this.UniformChecked = function (elementid) {
        $('#' + elementid + '').find('span:first').addClass('checked');
    }

    this.UniformUnChecked = function (elementid) {
        $('#' + elementid + '').find('span:first').removeClass('checked');
    }

    this.Checked = function (elementid) {
        $('#' + elementid + '').prop('checked', true);
    }

    this.UnChecked = function (elementid) {
        $('#' + elementid + '').prop('checked', false);
    }

    this.Disabled = function (elementid, TrueFalse) {
        $('#' + elementid + '').prop('disabled', TrueFalse);
    }


    this.AddClassById = function (elId, clas) {
        $('#' + elId + '').addClass(clas);
    }

    this.RemoveClassById = function (elId, clas) {
        $('#' + elId + '').removeClass(clas);
    }
    ...
    //start from here 
    this.loadCurrentYear = function () {
        debugger
        var currentYear = [];
        var thisYear = new Date().getFullYear();
        for (var y = thisYear; y >= 2000; y--) {
            currentYear.push(y);
        }
        return currentYear;
    };
