app.controller('ExportUndeliveryCtrl', ['$scope', '$http', '$window', 'crudService', 'conversion', '$filter', '$localStorage', 'uiGridConstants',
    function ($scope, $http, $window, crudService, conversion, $filter, $localStorage, uiGridConstants) {
        ...
        //Load Export Brand
        // Get all export brand
        $scope.ngModelBrand = '';
        $scope.listExportBrand = [];
        $scope.loadExportBrand = function () {
            debugger;
            $scope.listExportBrand = [];
            $scope.ngModelBrand = '';
            $("#ddlProductBrand").select2("data", { id: '', text: '--Select Brand--' });
            var apiRoute = sysCommonUrl + 'getExportBrand/';
            var param = { companyId: $scope.ngModelCompany == undefined ? '' : $scope.ngModelCompany };
            var listExportBrandList = crudService.getListByParam(apiRoute, param);
            listExportBrandList.then(function (response) {
                $scope.listExportBrand = response.data;
            },
                function (error) {
                    console.log("Error: " + error);
                });
        ...
        $scope.loadExportBrand();
        // Get all export brand        

        // Get Product By Company
        $scope.GetSKUByCompanyBrand = function () {

            $scope.ListProducts = [];
            $scope.ngModelProduct = '';
            $("#ddlProduct").select2("data", { id: '', text: '--Select Product--' });
            var param = { companyId: $scope.ngModelCompany == undefined || $scope.ngModelCompany == null ? '' : $scope.ngModelCompany, brandId: $scope.ngModelBrand == undefined || $scope.ngModelBrand == null ? '' : $scope.ngModelBrand };
            var apiRoute = sysCommonUrl + 'GetExportSKUByCompanyBrand/';
            var listModel = crudService.getListByParam(apiRoute, param);
            listModel.then(function (response) {
        ...
        $scope.ShowExportSales = function () {
            ...
            var commonQueryString = distributorId + ','
                + productId + ',' + brandId + ',' + transportId + ',' + startDate + ',' + endDate + ',' + requistionNo + ','
                + companyId + ',' + loggeduser + ',' + sardarId + ',' + locationId + ',' + reportType;
