app.controller('brandWiseSalesMonthYearCtrl', ['$scope', '$http', '$window', 'crudService', 'conversion', '$filter', '$localStorage', 'uiGridConstants',
    function ($scope, $http, $window, crudService, conversion, $filter, $localStorage, uiGridConstants) {
        ...
        frmName = 'formBrandWiseSalesStatement'; DelFunc = 'DeleteDistributorTargetMasterDetail'; DelMsg = 'DistributorTargetNo'; EditFunc = 'loadPIMasterDetailsByActivePI';
        ...
        $scope.ShowBrandWiseSalesMonthYear = function (data) {
            ...
            if (($scope.ReportType == 'party' || $scope.ReportType == 'pbs') && data == 4) {
                Command: toastr["warning"]("Please select other type !!!");
                return;
            }
            ...
            var queryStringBWSS = startDate + ',' + endDate + ',' + nationalId + ',' + divisionId + ',' + regionId + ',' + zoneId + ',' + distributorId + ',' + brandId + ',' + isBrandGroup + ',' + brandGroupId + ',' + reportFilter + ',' + data + ',' + $scope.ReportType + ',' + companyId;
        ...
        $scope.CommonQueryString = function (data) {
            ...
            commonQueryString = startDate + ',' + endDate + ',' + nationalId + ','
                + divisionId + ',' + regionId + ',' + zoneId + ',' + distributorId + ',' + brandId + ',' + isBrandGroup + ',' + brandGroupId + ',' + reportFilter;
        };
        ...
        // Get all brand by National and brand group
        $scope.loadBrandByCategoryBrandGroup = function () {
            ...
            //var mode = '1';
            //var nationalOid = $scope.ngModelNational == "" || $scope.ngModelNational == undefined ? null : $scope.ngModelNational;
            var categoryId = $scope.ngmProductCategory == "" || $scope.ngmProductCategory == undefined ? null : $scope.ngmProductCategory;
            var brndGroupId = $scope.brandGroupId == "" || $scope.brandGroupId == undefined ? null : $scope.brandGroupId;
            var userId = objcmnParam.loggeduser;
            var listBrandByCat = crudService.GetList(apiRoute, categoryId, brndGroupId, userId);
            listBrandByCat.then(function (response) {
                $scope.listBrandByCat = response.data;
            },
                function (error) {
        ...
        //$scope.loadBrandByCategoryBrandGroup();
        ...
        $scope.ShowDeliveryStatement = function () {
            ...
            var brandGroupId = $scope.brandGroupId == undefined ? '' : $scope.brandGroupId;
            var isBrandGroup = $scope.isBrandGroup ? '1' : '0';
            var queryStringUDS = commonQueryString + ',' + $scope.ngModelBrand + ',' + $scope.ngModelProduct + ',' + $scope.ngModelCompany + ',' + $scope.ngModelLocation + ',' + $scope.ReportType + ',' + isBrandGroup + ',' + brandGroupId;
        ...
        $scope.clear = function () {
            $scope.formBrandWiseSalesStatement.$setPristine();
            $scope.formBrandWiseSalesStatement.$setUntouched();
